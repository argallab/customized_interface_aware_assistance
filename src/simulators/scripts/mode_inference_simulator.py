...
class Simulator(object):
    def __init__(self, subject_id, assistance_block, block_id, training):
        ...
        if self.trial_info_dir_path is not None and os.path.exists(self.trial_info_dir_path) and not self.training:
            print ("LOAD METADATA")
            self.metadata_index_path = os.path.join(
                self.metadata_dir, self.testing_block_filename
            )  # metadata containing indices for the specific testing block
            ...
            with open(trial_info_filepath, "rb") as fp:
                trial_info_dict = pickle.load(
                    fp
                )  # this dict could have other info related to autonomy params. We are only interested in the environment params for the time being
            ...
            if not self.training:
                ...
                assert (
                    self.env_params["location_of_turn"] > 0
                    and self.env_params["location_of_turn"] <= self.env_params["num_turns"]
                )  # can't be the first or last location
            else:
