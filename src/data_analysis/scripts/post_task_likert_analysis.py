...
class PostTaskLikertAnalysis(object):
	...
	def get_percentages(self): 
		...
		labels = ['Strongly Agree', 'Agree', 'Somewhat Agree', 'Neutral', 'Somewhat Disagree', 'Disagree', 'Strongly Disagree']
		total_resondants = float(len(self.df)) # total number of people who repsonded to quesitonnaire

		for i in self.question_num: 

			q_responses = list()
			for j in labels: 
				reponse_percent = 100.0*len(self.df[self.df[i]==j].index.tolist())/total_resondants
