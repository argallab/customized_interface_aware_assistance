...
class ModeInferenceEnv(object):
    ...
    def _check_continuous_position_on_line_joining_waypoints(self, start_position, end_position, current_position):
        ...
        if not start_x == end_x:  # slope is not infinite #parallel to y axis
            ...
            curr_between = (min(start_x, end_x) <= curr_x <= max(start_x, end_x)) and (
                min(start_y, end_y) <= curr_y <= max(start_y, end_y)
            )  # check if the current point is between the end points
            curr_on_and_between = curr_on and curr_between  # Logical AND of ON and BETWEEN
        else:
            curr_on_and_between = (min(start_x, end_x) <= curr_x <= max(start_x, end_x)) and (
                min(start_y, end_y) <= curr_y <= max(start_y, end_y)
            )
        ...
        if curr_on_and_between:
            ...
            if (
                abs(curr_y - end_y) < 0.1 and abs(curr_x - end_x) < 0.1
            ):  # TODO (deepak.gopinath) Define these thresholds in utils.py folder.
    ...
    def _transform_continuous_position_to_discrete_position(self):
        ...
        start_index = int(
            self.current_discrete_state[0][-1]
        )  # get the number in the location id. That is, if location is p3, retrieve 3. This number is a proxy for which waypoint is the robot at
        # only consider line segments from the current index onwards.
        for (turn_id, p, pt) in zip(
            range(start_index, self.num_locations - 1),
            self.waypoints[start_index:-1],
            self.waypoints[start_index + 1 :],
        ):
            if turn_id < self.location_of_turn or turn_id > self.location_of_turn:
                position_on_line = self._check_continuous_position_on_line_joining_waypoints(
                    p, pt, current_position
                )  # check if the current position of the robot is at the START, BETWEEN or END of the line segment
    ...
    def _init_modes_in_which_motion_allowed_dict(self):
        ...
        for i, s in enumerate(self.LOCATIONS[:-1]):
            if self.start_direction == StartDirection.X:
                ...
                if i == self.location_of_turn:
                    self.MODES_MOTION_ALLOWED[s].append(
                        "t"
                    )  # note that the theta dimension is always the second one in the list.
        ...
        self.MODES_MOTION_ALLOWED[self.LOCATIONS[-1]] = self.MODES_MOTION_ALLOWED[self.LOCATIONS[-2]][
            0
        ]  # for the last location, copy the linear mode in which the motion is allowed for the second last location
    ...
    def _init_allowed_directions_of_motion(self):
        ...
        for i, (loc_p, loc_n) in enumerate(zip(self.LOCATIONS[:-1], self.LOCATIONS[1:])):
            diff_vector = np.array(self.LOCATIONS_TO_WAYPOINTS_DICT[loc_n]) - np.array(
                self.LOCATIONS_TO_WAYPOINTS_DICT[loc_p]
            )  # find the vector connecting two waypoints
            motion_dimension_index = np.nonzero(diff_vector)[
                0
            ]  # pick which is the dimension in which the vector is pointing. Same as the allowed dimension for motion
            sign_of_motion = np.sign(diff_vector[motion_dimension_index])[
                0
            ]  # determine the sign of the vector (that is if it is y, should the robot be moving up or down?)
            self.ALLOWED_DIRECTIONS_OF_MOTION[loc_p] = [sign_of_motion]
            if i == self.location_of_turn:
                if self.goal_orientation == PI / 2:
                    self.ALLOWED_DIRECTIONS_OF_MOTION[loc_p].append(
                        -1.0
                    )  # append direction indicator for rotation. counterclockwise is 1.0
                else:
                    self.ALLOWED_DIRECTIONS_OF_MOTION[loc_p].append(1.0)

        self.ALLOWED_DIRECTIONS_OF_MOTION[self.LOCATIONS[-1]] = [
            self.ALLOWED_DIRECTIONS_OF_MOTION[self.LOCATIONS[-2]][0]
        ]  # copy the direction of the linear dimension of the second last waypoint.
        ...
        for s in self.STATE_TRANSITION_MODEL.keys():  # for all states in the world
            for u in self.STATE_TRANSITION_MODEL[s].keys():  # for all available low-level commands
                if u == "Hard Puff" or u == "Hard Sip":
                    self.STATE_TRANSITION_MODEL[s][u] = (
                        s[0],
                        s[1],
                        MODE_SWITCH_TRANSITION[s[2]][u],
                    )  # generate new state
                if (
                    u == "Soft Puff" or u == "Soft Sip"
                ):  # if sp or ss, this will result in motion if the mode associated with s an allowed mode for motion
                    allowed_modes_for_motion = self.MODES_MOTION_ALLOWED[
                        s[0]
                    ]  # retrieve all the modes in which motion is allowed for this state. (note that, for the location in which turning happens there will be two modes in which motion is allowed)
                    self.STATE_TRANSITION_MODEL[s][u] = (
                        s[0],
                        s[1],
                        s[2],
                    )  # by default store the same state as next state. Because if no motion happens, the resultant state is also the same state
                    for m in allowed_modes_for_motion:
                        if (
                            m == s[2]
                        ):  # make sure that the allowed mode matches the mode in the state s. If it doesn't no motion will happen
                            if m != "t":  # allowed motion mode is a linear mode, x or y.
                                if TRANSITION_FOR_ACTION[rgc][u][m] == "next":
                                    new_loc_next = self.LOCATIONS[
                                        min(self.LOCATIONS.index(s[0]) + 1, len(self.LOCATIONS) - 1)
                                    ]
    ...
    def _create_optimal_next_state_dict(self):
        ...
        for s in self.STATES:
            if s[0] == self.LOCATIONS[-1]:  # no next state for the last location. Therefore, skip
                continue
            if (
                self.LOCATIONS.index(s[0]) < self.location_of_turn or self.LOCATIONS.index(s[0]) > self.location_of_turn
            ):  # if the location is before or after the location for turning, deal with it separately
                if s[2] not in self.MODES_MOTION_ALLOWED[s[0]]:
                    ...
                    self.OPTIMAL_NEXT_STATE_DICT[s] = (
                        self.LOCATIONS[min(self.LOCATIONS.index(s[0]) + 1, self.num_locations)],
                        s[1],
                        s[2],
                    )
                ...
                if s[2] != "t":
                    if (
                        s[1] != self.goal_orientation
                    ):  # if the current mode is not 'theta' switch to theta, if not turned yet.
                    ...
                    if s[1] != self.goal_orientation:  # if not turned yet
                        ...
                        self.OPTIMAL_NEXT_STATE_DICT[s] = (
                            s[0],
                            s[1] + diff_orientation,
                            s[2],
                        )  # go ahead and rotate in the proper direction
    ...
    def _render_goal(self):
        ...
        self.viewer.draw_line(
            self.goal_position,
            (
                self.goal_position[0] + 2 * (GOAL_RADIUS / SCALE) * math.cos(self.goal_orientation),
                self.goal_position[1] + 2 * (GOAL_RADIUS / SCALE) * math.sin(self.goal_orientation),
            ),
            linewidth=3.0,
        )
        ...
        for r in [self.robot]:
            ...
            for f in robot.fixtures:
                ...
                if type(f.shape) is b2CircleShape:
                    ...
                    if isinstance(r, RobotSE2):
                        if (
                            abs(np.linalg.norm(robot.linearVelocity)) > 0
                            or abs(np.linalg.norm(robot.angularVelocity)) > 0.0
                        ):
                            self.viewer.draw_circle(
                                f.shape.radius, 30, color=ROBOT_COLOR_WHEN_MOVING, filled=True
                            ).add_attr(t)
                        else:
    ...
    def _render_waypoints(self):
        ...
        for i in range(len(self.waypoints)):
            ...
            if robot_position[0] == self.waypoints[i][0] and robot_position[1] == self.waypoints[i][1]:
                self.viewer.draw_circle(WP_RADIUS / SCALE, 30, True, color=(0, 1, 0)).add_attr(
                    t
                )  # light up the waypoint if the robot is EXACTLY at the waypoint. Feedback for the user
            else:
    ...
    def _render_turn_location(self):
        ...
        self.viewer.draw_circle(8 * WP_RADIUS / SCALE, 4, True, color=TURN_LOCATION_COLOR).add_attr(
            t
        )  # TODO Look into how to properly render a box instead of a circle with 4 points!
    ...
    def _render_start_end_text(self):
        if self.r_to_g_relative_orientation == RGOrient.TOP_RIGHT:
            if self.start_direction == StartDirection.X:
                start_text_location = (
                    self.robot_position[0] * SCALE - START_GOAL_TEXT_DISPLACEMENT,
                    self.robot_position[1] * SCALE,
                )
                goal_text_location = (
                    self.goal_position[0] * SCALE - START_GOAL_TEXT_DISPLACEMENT,
                ...
                goal_text_location = (
                    ...
                    self.goal_position[1] * SCALE - START_GOAL_TEXT_DISPLACEMENT,
                )
            ...
            if self.start_direction == StartDirection.X:
                start_text_location = (
                    self.robot_position[0] * SCALE,
                    self.robot_position[1] * SCALE - START_GOAL_TEXT_DISPLACEMENT,
                )
                goal_text_location = (
                    self.goal_position[0] * SCALE - START_GOAL_TEXT_DISPLACEMENT,
                    self.goal_position[1] * SCALE,
        ...
        self.viewer.draw_text(
            "Start",
            x=start_text_location[0],
            y=start_text_location[1],
            font_size=TRIAL_OVER_TEXT_FONTSIZE - 6,
            color=(0, 0, 0, 255),
            bold=False,
        )
        self.viewer.draw_text(
            "Goal",
            x=goal_text_location[0],
            y=goal_text_location[1],
            font_size=TRIAL_OVER_TEXT_FONTSIZE - 6,
            color=(0, 0, 0, 255),
            bold=False,
        )
    ...
    def _render_trial_over_text(self):
        self.viewer.draw_text(
            "TRIAL OVER",
            x=TRIAL_OVER_TEXT_DISPLAY_POSITION[0],
            y=TRIAL_OVER_TEXT_DISPLAY_POSITION[1],
            font_size=TRIAL_OVER_TEXT_FONTSIZE,
            color=TRIAL_OVER_TEXT_COLOR,
            anchor_y=TRIAL_OVER_TEXT_Y_ANCHOR,
            bold=True,
        )
    ...
    def _generate_path(self):
        ...
        """
        if (
            self.r_to_g_relative_orientation == RGOrient.TOP_RIGHT
            or self.r_to_g_relative_orientation == RGOrient.BOTTOM_LEFT
        ):
            left_edge_angle = PI / 2 + PI / 4.0
            right_edge_angle = -PI / 4.0
        elif (
            self.r_to_g_relative_orientation == RGOrient.TOP_LEFT
            or self.r_to_g_relative_orientation == RGOrient.BOTTOM_RIGHT
        ):
            left_edge_angle = -(PI / 2 + PI / 4.0)
            right_edge_angle = PI / 4.0
        ...
        for i in range(0, self.num_turns + 2):
            self.path_points[i][0] = self.waypoints[i] + (
                r * math.cos(left_edge_angle),
                r * math.sin(left_edge_angle),
            )  # left edge
            self.path_points[i][1] = self.waypoints[i] + (
                r * math.cos(right_edge_angle),
                r * math.sin(right_edge_angle),
            )
    ...
    def reset(self):
        ...
        self.r_to_g_relative_orientation = self.env_params[
            "r_to_g_relative_orientation"
        ]  # top right, top left, bottom, right, bottom left. relative position of the goal with respect to starting position of robot
        ...
        self.STATES = [
            s for s in itertools.product(self.LOCATIONS, self.ORIENTATIONS, self.DIMENSIONS)
        ]  # list of all discrete states for the given configuration
        self.waypoints = np.zeros((self.num_locations, 2))
    ...
    def step(self, input_action):
        ...
        (
            current_discrete_position,
            should_snap,
        ) = (
            self._transform_continuous_position_to_discrete_position()
        )  # transform the continuous robot position to a discrete state representation. Easier for computing the optimal action etc
        if should_snap:
            ...
            self.robot.set_position(
                self.LOCATIONS_TO_WAYPOINTS_DICT[current_discrete_position]
            )  # directly set the position of the robot
            (
                current_discrete_position,
                should_snap,
            ) = self._transform_continuous_position_to_discrete_position()  # recompute the discrete position

        (
            current_discrete_orientation,
            should_snap,
        ) = (
            self._transform_continuous_orientation_to_discrete_orientation()
        )  # compute discrete orientation state from continuous orientation of the robot
        if should_snap:
            self.robot.set_orientation(
                current_discrete_orientation
            )  # if snap is true then force orientation to be that of the target orientation
            (
                current_discrete_orientation,
                should_snap,
            ) = self._transform_continuous_orientation_to_discrete_orientation()  # recompute the discrete orientation
        ...
        self.current_discrete_state = (
            ...
            current_discrete_orientation,
            self.current_mode,
        )  # update the current discrete state.
        current_allowed_mode = (
            self._retrieve_current_allowed_mode()
        )  # x,y,t #for the given location, retrieve what is the allowed mode of motion.
        current_allowed_mode_index = DIM_TO_MODE_INDEX[
            current_allowed_mode
        ...
        user_vel[
            np.setdiff1d(self.DIMENSION_INDICES, current_allowed_mode_index)
        ] = 0.0  # zero out all velocities except the ones for the allowed mode
        ...
        if (
            get_sign_of_number(user_vel[current_allowed_mode_index])
            != current_allowed_direction_of_motion_in_allowed_mode
        ):  # check if the direction velocity component in the allowed mode matches the allowed direction of motion in the allowed mode
        ...
        return (
            ...
            self.current_discrete_state,
            is_done,
        )  # slightly hacky for velocities. TODO fix this. for some reason getters for velocities were not working properly
