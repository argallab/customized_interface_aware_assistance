...
class SimulationAnalysis(object):
    ...
    def _compute_percentage_of_intervened(self, force_compute=False):
        ...
        only_corrective_and_filtered_subset_pkl_name = os.path.join(
            subset_path, "only_corrective_and_filtered_subset.pkl"
        )
        ...
        ui_a_vs_um_ui_matrix_filename = os.path.join(
            results_dir_path, "ui_a_vs_um_ui_matrix_for_u_inferred_match_with_ground_truth.pkl"
        )
        did_assistance_intervene_list_filename = os.path.join(
            results_dir_path, "u_inferred_match_with_ground_truth_list.pkl"
        )
        if (
            os.path.exists(ui_a_vs_um_ui_matrix_filename) and not force_compute
            ...
            for i, ui_a in enumerate(self.UI_GIVEN_A_NOISE):
                for j, um_ui in enumerate(self.UM_GIVEN_UI_NOISE):
                    percentage_of_assistive_interventions = float(
                        sum(did_assistance_intervene_list[ui_a][um_ui])
                    ) / len(did_assistance_intervene_list[ui_a][um_ui])
    ...
    def _compute_percentage_of_intervened_old(self, force_compute=False):
        ...
        subset_path = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "subsets"
        )  # mayebe mvoe this to main function
        if not os.path.exists(subset_path):
            os.makedirs(subset_path)
        compare_mode_switches_between_assistance_conditions_pklnames = os.path.join(
            subset_path, "is_intervened_pklnames.pkl"
        )
        ...
        ui_a_vs_um_ui_matrix_for_u_intended_not_equals_um_filename = os.path.join(
            results_dir_path, "ui_a_vs_um_ui_matrix_for_u_intended_not_equals_um.pkl"
        )
        ui_a_vs_um_ui_matrix_for_normalized_entropy_filename = os.path.join(
            results_dir_path, "ui_a_vs_um_ui_matrix_for_normalized_entropy.pkl"
        )
        ui_a_vs_um_ui_matrix_for_is_intervened_filename = os.path.join(
            results_dir_path, "ui_a_vs_um_ui_matrix_for_is_intervened.pkl"
        )
        ui_a_vs_um_ui_matrix_for_is_intervened_after_u_intended_not_equals_um_filename = os.path.join(
            results_dir_path, "ui_a_vs_um_ui_matrix_for_is_intervened_after_u_intended_not_equals_um.pkl"
        )
        ui_a_vs_um_ui_matrix_for_is_normalized_entropy_less_than_threshold_filename = os.path.join(
            results_dir_path, "ui_a_vs_um_ui_matrix_for_is_normalized_entropy_less_than_threshold.pkl"
        )
        ...
        is_intervened_after_u_intended_not_equals_um_list_filename = os.path.join(
            results_dir_path, "is_intervened_after_u_intended_not_equals_um_list.pkl"
        )
        is_normalized_entropy_less_than_threshold_list_filename = os.path.join(
            results_dir_path, "is_normalized_entropy_less_than_threshold_list.pkl"
        )
        ...
        if os.path.exists(u_intended_not_equals_um_list_filename) and not force_compute:
            ...
            for k, sf in enumerate(sim_files_that_satisfies_criteria):
                ...
                is_normalized_entropy_less_than_threshold = simulation_result["data"][
                ...
                is_intervened_list[ui_a][um_ui].extend(
                    [a and not b for a, b in zip(is_normalized_entropy_less_than_threshold, u_intended_equals_um)]
                )  # overall percentage
                u_intended_not_equals_um_list[ui_a][um_ui].extend([not a for a in u_intended_equals_um])
                is_normalized_entropy_less_than_threshold_list[ui_a][um_ui].extend(
                    is_normalized_entropy_less_than_threshold
                )
                temp_list = []
            ...
            ui_a_vs_um_ui_matrix_for_u_intended_not_equals_um = np.zeros(
                (len(self.UI_GIVEN_A_NOISE), len(self.UM_GIVEN_UI_NOISE))
            )
            ui_a_vs_um_ui_matrix_for_normalized_entropy = np.zeros(
                (len(self.UI_GIVEN_A_NOISE), len(self.UM_GIVEN_UI_NOISE))
            )
            ui_a_vs_um_ui_matrix_for_is_intervened = np.zeros((len(self.UI_GIVEN_A_NOISE), len(self.UM_GIVEN_UI_NOISE)))
            ui_a_vs_um_ui_matrix_for_is_intervened_after_u_intended_not_equals_um = np.zeros(
                (len(self.UI_GIVEN_A_NOISE), len(self.UM_GIVEN_UI_NOISE))
            )
            ui_a_vs_um_ui_matrix_for_is_normalized_entropy_less_than_threshold = np.zeros(
                (len(self.UI_GIVEN_A_NOISE), len(self.UM_GIVEN_UI_NOISE))
            )
            ...
            for i, ui_a in enumerate(self.UI_GIVEN_A_NOISE):
                for j, um_ui in enumerate(self.UM_GIVEN_UI_NOISE):
                    print(ui_a, um_ui)
                    average_normalized_entropy = float(sum(normalized_entropy_list[ui_a][um_ui])) / len(
                        normalized_entropy_list[ui_a][um_ui]
                    )
                    percentage_normalized_entropy_less_than_threshold = float(
                        sum(is_normalized_entropy_less_than_threshold_list[ui_a][um_ui])
                    ) / len(is_normalized_entropy_less_than_threshold_list[ui_a][um_ui])
                    percentage_is_intervened = float(sum(is_intervened_list[ui_a][um_ui])) / len(
                        is_intervened_list[ui_a][um_ui]
                    )
                    percentage_u_intended_not_equals_um = float(sum(u_intended_not_equals_um_list[ui_a][um_ui])) / len(
                        u_intended_not_equals_um_list[ui_a][um_ui]
                    )
                    ...
                    ui_a_vs_um_ui_matrix_for_is_intervened_after_u_intended_not_equals_um[
                        i, j
                    ] = percentage_is_intervened_after_u_intended_not_equals_um
                    ui_a_vs_um_ui_matrix_for_is_normalized_entropy_less_than_threshold[
                        i, j
                    ] = percentage_normalized_entropy_less_than_threshold
            ...
            print(
                "ui_a_vs_um_ui_matrix_for_u_intended_not_equals_um", ui_a_vs_um_ui_matrix_for_u_intended_not_equals_um
            )
    ...
    def _compare_mode_switches_between_assistance_conditions(self, force_compute=False):
        ...
        if not os.path.exists(subset_path):
            os.makedirs(subset_path)
        compare_mode_switches_between_assistance_conditions_pklnames = os.path.join(
            subset_path, "compare_mode_switches_between_assistance_conditions_pklnames.pkl"
        )
        if os.path.exists(compare_mode_switches_between_assistance_conditions_pklnames):
            with open(compare_mode_switches_between_assistance_conditions_pklnames, "rb") as fp:
                sim_files_that_satisfies_criteria = pickle.load(fp)
        ...
        ui_a_vs_um_ui_matrix_for_total_steps_filename = os.path.join(
            results_dir_path, "ui_a_vs_um_ui_matrix_for_total_steps.pkl"
        )
                ...
                for j, um_ui in enumerate(self.UM_GIVEN_UI_NOISE):
                    ...
                    for k, sf in enumerate(sim_files_that_satisfies_criteria):
                        with open(sf, "rb") as fp:
                            simulation_result = pickle.load(fp)
                        if (
                            simulation_result["combination_dict"]["ui_given_a_noise"] == ui_a
                            and simulation_result["combination_dict"]["um_given_ui_noise"] == um_ui
                        ):
                            ui_a_vs_um_ui_matrix_for_total_steps[ui_a][um_ui][
                                simulation_result["combination_dict"]["assistance_type"]
                            ].append(simulation_result["data"]["total_steps"])
        ...
        for i, ui_a in enumerate(self.UI_GIVEN_A_NOISE):
            ...
            for j, um_ui in enumerate(self.UM_GIVEN_UI_NOISE):
                corrective_list_means.append(
                    np.mean(ui_a_vs_um_ui_matrix_for_total_steps[ui_a][um_ui][AssistanceType.Corrective])
                )
                filter_list_means.append(
                    np.mean(ui_a_vs_um_ui_matrix_for_total_steps[ui_a][um_ui][AssistanceType.Filter])
                )
                no_assistance_list_means.append(
                    np.mean(ui_a_vs_um_ui_matrix_for_total_steps[ui_a][um_ui][AssistanceType.No_Assistance])
                )
            fig, ax = plt.subplots()
            N = 4
            ind = np.arange(N) * 3
            ...
            ax.set_title("Internal Model Noise Level = {}".format(str(ui_a)), fontsize=24)
            ax.set_xticks(ind + 1.5 * width)
            ax.set_xticklabels(("0.1", "0.3", "0.5", "0.7"))
            ax.set_ylabel("Total Number of Steps", fontsize=20)
            ax.set_xlabel("User Input Distortion Level", fontsize=20)
            ax.set_ylim((0, 40))

            ax.legend((p1[0], p2[0], p3[0]), ("No_Assistance", "Filter", "Corrective"), loc="upper left")
            ax.autoscale_view()
            plt.show()
    ...
    def _check_accuracy_of_corrected_um(self, force_compute=False):
        ...
        u_inferred_match_with_ground_truth_list_filename = os.path.join(
            results_dir_path, "u_inferred_match_with_ground_truth_list.pkl"
        )
        if (
            os.path.exists(ui_a_vs_um_ui_matrix_filename) and not force_compute
            ...
            for k, sf in enumerate(sim_files_that_satisfies_criteria):
                ...
                u_inferred_match_with_ground_truth_list[ui_a][um_ui].extend(
                    simulation_result["data"]["u_inferred_match_with_ground_truth"]
                )
                # else:
                #     print('Skip')
            ...
            for i, ui_a in enumerate(self.UI_GIVEN_A_NOISE):
                for j, um_ui in enumerate(self.UM_GIVEN_UI_NOISE):
                    percentage_of_correct_inference = float(
                        sum(u_inferred_match_with_ground_truth_list[ui_a][um_ui])
                    ) / len(u_inferred_match_with_ground_truth_list[ui_a][um_ui])
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=os.path.join(
            ...
            "simulation_results_with_higher_entropy_with_proper_noise",
        ),
