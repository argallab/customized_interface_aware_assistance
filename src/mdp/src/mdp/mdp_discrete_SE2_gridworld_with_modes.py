...
class MDPDiscreteSE2GridWorldWithModes(DiscreteMDP):
    ...
    def _define_mdp(self):
        ...
        self.obstacles = self.env_params['mdp_obstacles'] #list of 2d. If 2d state, then implies, all possible modes and orientations are obstacle for the location
        self.goal_state = self.env_params['mdp_goal_state'] #3d (x, y, t)
        self.robot_type = self.env_params['robot_type'] #SE2
        self.mode_set_type = self.env_params['mode_set_type'] # assert this is one D. As the task level actions are for 1D interfaces. 
        ...
        self.state_coords = list(itertools.product(range(self.width), range(self.height), range(self.num_discrete_orientations), self.mode_set.keys()))
        ...
        if self.vs_and_policy_dict is not None and 'R_matrix' in self.vs_and_policy_dict:
            ...
            for action_vector in self.task_level_actions.values():
                ...
                for state_coord in self.state_coords:
                    ...
                    elif transition_type == TransitionType.VALID and self._check_if_state_coord_is_goal_state(new_state_coord): #its a valid transition into goal
                        R[state_id, new_state_id] = self.goal_reward * action_goal_weight
                    elif transition_type == TransitionType.VALID: #valid transitionn into adjacent valid state that is not the goal state
    ...
    def _transition(self, state_coord, task_level_action):
        if self._check_in_obstacle(state_coord):
            ...
            if vel_tuple != (0,0,0):
                ...
                if new_state_coord[Dim.X.value] < 0 or new_state_coord[Dim.X.value] > self.width - 1 or new_state_coord[Dim.Y.value] < 0 or new_state_coord[Dim.Y.value] > self.height - 1:
                ...
                new_state_coord = [state_coord[Dim.X.value], state_coord[Dim.Y.value], state_coord[Dim.Theta.value], self._get_mode_transition(current_mode, mode_switch_command)]
                transition_type = TransitionType.VALID
    ...
    def create_action_dict(self):
        ...
        self.task_level_actions['to_mode_r'] = (2, 'to_mode_r', 10,1) #penalize mode switch more than real motion, therefore the action weight is 10
    ...
    def _convert_grid_coords_to_1D_state(self, coord): #coord can be a tuple, list or np.array
        ...
        state_id = ((x_coord*self.height + y_coord)*self.num_discrete_orientations + theta_coord)*self.num_modes + mode - 1
        return state_id
